...
def pose_check(
    ...
    # Enrollment uses a stricter det-score floor than live recognition so
    # ghost faces never enter the gallery.
    min_det_score = max(
        get_float(db, "engine.min_det_score"),
        get_float(db, "enroll.min_det_score"),
    )
    result = enrollment_svc.pose_check(
        frame, step, get_provider(),
        min_face_ratio=min_face_ratio, min_det_score=min_det_score,
    )
