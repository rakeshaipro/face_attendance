...
def assess_quality(
    ...
    *,
    ...
    # Empirical: <50 = very blurry, >120 = crisp.
    sharpness = _clamp(lap_var / 120.0)

    # --- brightness ---------------------------------------------------
    # Mean grayscale intensity (0–255); ideal ~110.
    brightness = float(np.mean(gray))
    # Ideal range: 70–180 covers most indoor lighting and darker skin tones.
    # Hard floor at 40 (near-dark) and 210 (blown out).
    # Gentle penalty: centred at 90 (warm indoor), full width 150.
    if brightness < 40 or brightness > 210:
        bright_score = 0.2
    else:
        bright_score = 1.0 - abs(brightness - 90.0) / 150.0
        bright_score = _clamp(bright_score)

    score = round(0.45 * sharpness + 0.20 * bright_score + 0.35 * size_score, 4)
    acceptable = (face_ratio >= min_face_ratio) and sharpness > 0.1 and bright_score > 0.1
