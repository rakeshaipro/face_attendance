...
# When an installation still holds a previous *default* that we later
# re-tuned for accuracy, upgrade it in place. User-customised values
# (anything other than the listed old default) are left untouched.
_DEFAULT_MIGRATIONS: list[tuple[str, str, str]] = [
    # (key, old_default, new_default)
    ("engine.similarity_threshold", "0.60", "0.45"),
    ("engine.min_face_ratio", "0.10", "0.08"),
    ("engine.match_confirm_window", "1", "2"),
    ("enroll.quality_threshold", "0.30", "0.40"),
    ("enroll.min_overall_quality", "0.40", "0.50"),
]
...
def seed_defaults(db: Session) -> int:
    ...
    Also applies accuracy-tuning migrations for keys that are still at a
    previous known default (so existing installs pick up the new sweet
    spot without clobbering operator overrides).
    """
    seeded = 0
    ...
    migrated = 0
    for key, old, new in _DEFAULT_MIGRATIONS:
        row = db.execute(select(SystemSetting).where(SystemSetting.key == key)).scalar_one_or_none()
