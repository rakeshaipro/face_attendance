...
def test_resolve_providers_defaults_to_cpu_for_negative_ctx():
    """ctx_id < 0 always selects CPU-only, regardless of what ORT reports."""
    from app.engine.face_provider import _resolve_providers
    assert _resolve_providers(-1) == ["CPUExecutionProvider"]


def test_resolve_providers_cpu_when_no_gpu_wheel():
    """When only the CPU wheel is installed, even a GPU request yields CPU."""
    from app.engine.face_provider import _resolve_providers
    # The CPU wheel (onnxruntime) lacks CUDAExecutionProvider.
    providers = _resolve_providers(0)
    assert "CPUExecutionProvider" in providers
    assert "CUDAExecutionProvider" not in providers


def test_fake_provider_honors_min_det_score():
    """The fake provider must filter by min_det_score like the real one."""
    import numpy as np

    from tests.fake_provider import FakeFaceProvider, make_frame

    provider = FakeFaceProvider(score=0.40)
    frame = make_frame()
    # Below floor → no faces.
    assert provider.detect(frame, min_det_score=0.50) == []
    # At/above floor → one face.
    det = provider.detect(frame, min_det_score=0.40)
    assert len(det) == 1
    assert det[0].score == 0.40
